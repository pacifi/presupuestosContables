...
import ppm.contable.proyecto.modelo.ModeloPresupuestoMateriales;
...
public class MateriaPrimaController {
   ...
   @Autowired
    public ProductoConsumoServicio productoConsumo;
    
    @RequestMapping(value = "presupuestoMateriales", method = RequestMethod.GET)
    public ModelAndView PresupuestoMateriales() {
        ...
        Double materiaPrimaRequerida= 0.0;
  ...
  Double polInventarioFin=Double.parseDouble(String.valueOf(productoConsumo.buscarProductoConsumoId(idProducto).getIdProducto()));
  ModeloPresupuestoMateriales materiales;
  List<ModeloPresupuestoMateriales> listMa=new ArrayList<ModeloPresupuestoMateriales>();
        for (int j = 0; j < listaProducto.size(); j++) {
            ...
            for (int i = 0; i < pronosticoVentasList.size(); i++) {
                ...
                System.out.println("MATERIALES Resultado del mes"+i+" : "+resultado);
        ...
        return new ModelAndView("contable/presupuestos/PresupuestoMateriales", modelo);
